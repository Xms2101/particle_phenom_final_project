...
# Key parameters
bins = np.linspace(2, 9, 14) #Make sure these divide well!! Do the math!!
...
mc_files   = ["data/monte_carlo/1p_mc_snip_7.root"]
...
pi_E_mc_truth_MeV        = mc_arr["truth_pi_E"] ##########
pi_E_mc_truth            = pi_E_mc_truth_MeV / MEV_TO_GEV
...
TRUTH_all = pi_E_mc_truth
RECO_all  = pi_E_mc_reco

truth_for_response = TRUTH_all[mc_detector_mask]
reco_for_response  = RECO_all[mc_detector_mask]
print(truth_for_response)
#truth_for_response = truth_for_response.flatten()
#reco_for_response  = reco_for_response.flatten()
...
# -------------------------
data_hist, _ = np.histogram(pi_E_r, bins=bins)   # observed data (detector-level; no physics cuts here)
...
signal_truth = pi_E_mc_truth[physics_signal_mask_truth]
...
ax.set_xlabel("Truth pion energy [GeV]")
...
ax.set_xlabel("Reconstructed pion energy [GeV]")
ax.set_ylabel("Truth pion energy [GeV]")
...
ax.errorbar(bin_centers, unf_scaled, yerr=unf_err_scaled, fmt='o', label='Unfolded (scaled)', ms=6, capsize=3, color='C3')
